...
// const contactInfo = [];
const contactInfo = graduates.map(function(arr){
  return `${arr.first_name} ${arr.email}`;

  let filterNamEm = `${arr.first_name} ${arr.email}`;
  contactInfo.push(filterNamEm);
...
const unisWithUni = [];
// graduates.map(function(uniStr){
//   return universities.push(uniStr.university);
// }); *****With out this commented out, it returned repeats. I don't know why I was getting repeats.*******
...
*/
const lowPopulationAnimals = zooAnimals.filter(function(smallPop){
...
*/
const populationTotal = zooAnimals.reduce(function(accu, num ){
  return accu + num.population
}, 0);
